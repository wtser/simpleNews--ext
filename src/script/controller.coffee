...
.controller('OptionCtrl', ($scope) ->
  ...
  optionList.domain = 'https://ten-read.b0.upaiyun.com/data/'
  ...
  myList.importAction = ->
    if confirm('确认导入？导入会覆盖原有数据,且无法恢复')
      if myList.sitesImport
        myList.data = JSON.parse myList.sitesImport
        localStorage.setItem 'sites', JSON.stringify(myList.data)
        myList.cancel()
